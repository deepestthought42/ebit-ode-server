...
** [2018-09-06 Thu 12:44] starting GUI
- lets start by including the old gui code
  #+BEGIN_SRC lisp
  (with-project-dir:with-project-dir ("/home/renee/phd/src/ebit-evolution.project/")
    (ql:quickload :rkcustomplot))
#+END_SRC  
- ok, weird error message for qt-libs, probably have to update qt-libs
- nothing makes sense here, maybe we just use qwt ... doesn't look as nice but whatever
- lets first try qtools in a freshing lisp image
  #+BEGIN_SRC lisp
  (ql:quickload :with-project-dir)
  (ql:quickload :fare-quasiquote)
  (ql:quickload :puri-ppcre)
#+END_SRC
