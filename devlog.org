...
(defun test (port)
  (let+ ((system (make-instance 'eir:ebit-system
				:beam-radius-in-um 100d0
				:initial-populations '((:a 19 :z 8 :q 1 :no 10000d0 :temp-in-ev 5d0))
				:nuclides (list (nubase:get-entry-for 19 8))))
...
(let+ ((res-msg (test 2006))
       ((&slots ebitodemessages:values ebitodemessages:times) (ebitodemessages:ode-result res-msg)))
  ,#+nil
  ...
  (labels ((cmd (fmt-str &rest args)
    ...
    (mgl-gnuplot:with-session ()
      ...
      (mgl-gnuplot:plot*
       (iter
	 (for v in ebitodemessages:values)
	 (collect
	     (mgl-gnuplot:data*
	      (iter
		...
		(collect (list tt y)))
	      (if (= 0 (ebitodemessages:q (ebitodemessages:index v)))
		  (format nil "with lines dt ~D title '^{~D}~a'"
			  dt
			  (ebitodemessages:a (ebitodemessages:index v)) 
			  (ionization-energies:get-name-for-z (ebitodemessages:z (ebitodemessages:index v))))
